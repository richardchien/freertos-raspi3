...
static volatile BCM2835_INTC_REGS *const pRegs =
	(BCM2835_INTC_REGS *)(BCM2835_BASE_INTC);
...
static void handleRange(unsigned long pending, const unsigned int base)
{
	while (pending) {
		...
		if (g_VectorTable[irq].pfnHandler)
			g_VectorTable[irq].pfnHandler(
				irq, g_VectorTable[irq].pParam);
...
void irqRegister(const unsigned int irq, FN_INTERRUPT_HANDLER pfnHandler,
		 void *pParam)
{
